module Spree
  module ProductDecorator
    def self.prepended(base)
      ...
      def available_options(option_values = nil)
        if option_values
          ids = option_values.map(&:to_i)
          puts variant_ids = self.option_value_variants.where(option_value_id: ids).group(:variant_id).having("count(variant_id) = #{ids.count}").pluck(:variant_id)
          return Spree::OptionValue.joins(:option_value_variants).where("variant_id IN (?)", variant_ids).group(:name, :option_type_id)
        else
          return self.option_values.order('spree_option_values.position, spree_option_values.id').group(:name, :option_type_id)
        end
      end

      def options_hash(option_values = nil)
        h = {}
        position = 0
        for ot in self.option_types
          h[ot.id] = {presentation: ot.presentation, values: [], position:  position += 1}
        end
        for option in self.available_options(option_values)
